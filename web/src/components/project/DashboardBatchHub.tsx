...
import { apiClient, GeminiPoolStatus, GlobalPipelineSettings } from '../../api/client';
...
export const DashboardBatchHub: React.FC<DashboardBatchHubProps> = ({
  ...
  const [pipelineSettings, setPipelineSettings] = useState<GlobalPipelineSettings | null>(null);

  const fetchPipelineSettings = useCallback(async () => {
    try {
      const res = await apiClient.getPipelineSettings();
      setPipelineSettings(res);
    } catch {}
  }, []);
  ...
  }, [fetchPipelineSettings, fetchGeminiPoolStatus]);
              ...
              <div className="space-y-4 text-xs">
                <div className="flex items-center justify-between">
                  ...
                  <button
                    ...
                    className="text-indigo-400 hover:text-indigo-300 font-semibold text-[11px] flex items-center gap-1 transition cursor-pointer"
                  >
                    <Settings className="w-3 h-3" />
                ...
                <div className="space-y-2 p-2.5 rounded-lg bg-slate-950 border border-slate-800 text-[11px]">
                  <div className="flex items-center justify-between">
                    ...
                    <span className="font-semibold text-cyan-300">
                      {pipelineSettings?.ocr.engine === 'paddle' ? 'PaddleOCR v6' : 'RapidOCR ONNX (Local)'}
                    </span>
                  </div>
                  <div className="flex items-center justify-between">
                    <span className="text-slate-400">Dịch thuật:</span>
                    <span className="font-semibold text-amber-300 truncate max-w-[150px]" title={pipelineSettings?.translation.gemini_model}>
                      {pipelineSettings?.translation.provider === 'google_web'
                        ? 'Google Web'
                        : `${pipelineSettings?.translation.gemini_model || 'Gemini 2.5 Flash'}`}
                    </span>
                  </div>
                  <div className="flex items-center justify-between">
                    <span className="text-slate-400">Thuyết minh (TTS):</span>
                    <span className="font-semibold text-emerald-300">
                      {pipelineSettings?.dubbing.voice?.includes('HoaiMy') ? 'Hoài My (Nữ)' : 'Nam Minh (Nam)'}
                    </span>
                  </div>
                  <div className="flex items-center justify-between">
                    <span className="text-slate-400">Render phần cứng:</span>
                    <span className="font-semibold text-purple-300">
                      {pipelineSettings?.render.ffmpeg_encoder === 'cpu' ? 'CPU libx264' : 'FFmpeg NVENC / Auto'}
                    </span>
                  </div>
                </div>
                ...
                <div className="space-y-1.5">
                  <div className="flex items-center justify-between">
                    ...
                    <span className="text-[10px] text-slate-500 font-mono">Tự lưu toàn cục</span>
                  </div>
                  <select
                    className="w-full bg-slate-950 border border-slate-800 rounded p-2 text-slate-200 text-xs"
                    value={pipelineSettings?.dubbing.voice || 'vi-VN-NamMinhNeural'}
                    onChange={async (e) => {
                      const newVoice = e.target.value;
                      if (pipelineSettings) {
                        const next = { ...pipelineSettings, dubbing: { ...pipelineSettings.dubbing, voice: newVoice } };
                        setPipelineSettings(next);
                        try {
                          await apiClient.savePipelineSettings(next);
                        } catch {}
                      }
                    }}
                  >
