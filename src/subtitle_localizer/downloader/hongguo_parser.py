...
def get_device_keys() -> Dict[str, str]:
    ...
    if not device_id or not install_id or device_id == "0" or install_id == "0":
        # Fallback to pre-registered valid ByteDance credentials if corrupted/empty
        device_id = "2222566783375657"
        install_id = "2222566783379753"
...
def save_device_keys(device_id: str, install_id: str, platform: str = "android") -> Dict[str, str]:
    """Save custom or updated device keys to config.json."""
    config_path = get_runtime_base_dir() / "config.json"
    try:
    ...
    existing["install_id"] = str(install_id).strip()
...
def get_device_status_info() -> Dict[str, Any]:
    """Return full device profile info for UI inspection."""
    try:
        keys = get_device_keys()
    except Exception:
        keys = {"device_id": "", "install_id": "", "platform": "android"}
    ...
    if config_path.exists():
        try:
            ...
            last_updated = time.strftime("%H:%M:%S %d/%m/%Y", time.localtime(mtime))
        except Exception:
            pass

    return {
        "device_id": keys.get("device_id", ""),
        "install_id": keys.get("install_id", ""),
        "platform": keys.get("platform", "android"),
        "device_brand": "Xiaomi",
        "device_model": "MI 12 (Android 10)",
        "status": "ready" if keys.get("device_id") else "unconfigured",
        "server_source": "https://log.snssdk.com (ByteDance SNSSDK)",
    ...
    try:
        try:
            ...
            try:
                ...
                from subtitle_localizer.downloader.liushen.device_register import device_register
        new_config = device_register(proxy=proxy)
        dev_id = str(new_config.get("device_id", "")).strip() if new_config else ""
        iid = str(new_config.get("install_id", "")).strip() if new_config else ""
        if dev_id and dev_id != "0" and len(dev_id) > 5:
            ...
            try:
                ...
                existing = {}
            existing["device_id"] = dev_id
            existing["install_id"] = iid
            existing["platform"] = new_config.get("platform", "android")
            config_path.write_text(json.dumps(existing, indent=2, ensure_ascii=False), encoding="utf-8")
            ...
            print(f"[device] rotate_device got unconfirmed device_id='{dev_id}', keeping existing keys")
    except Exception as exc:
        print(f"[device] rotate_device_error={exc}")
    return get_device_keys()


def replace_failed_device(device_id: str, platform: str, proxy: Optional[str] = None) -> None:
    """Auto-register a new device when the current one is shadow-banned."""
    rotate_device(proxy=proxy)
...
def resolve_video_url(
    ...
    for attempt in range(max_retries):
        current_device_keys = device_keys if (device_keys and attempt == 0) else get_device_keys()
        ...
        try:
            ...
            print(f"[retry] attempt={attempt+1} backoff={backoff:.1f}s error={exc}")
            time.sleep(backoff)
            continue
